...
static Sym *gnu_lookup(uint32_t h1, uint32_t *hashtab, struct dso *dso, const char *s) {
  ...
  for (h1 |= 1;; i++) {
    uint32_t h2 = *hashval++;
    if ((h1 == (h2 | 1)) && (!dso->versym || dso->versym[i] >= 0) && !strcmp(s, dso->strings + dso->syms[i].st_name)) return dso->syms + i;
    if (h2 & 1) break;
  }
  ...
  if (DL_FDPIC && !(eh->e_flags & FDPIC_CONSTDISP_FLAG)) {
    ...
    for (ph = ph0, i = 0; i < nsegs; ph = (void *)((char *)ph + eh->e_phentsize)) {
      ...
      if (prot & PROT_WRITE) {
        ...
        if (pgend > pgbrk && mmap_fixed(map + pgbrk, pgend - pgbrk, prot, MAP_PRIVATE | MAP_FIXED | MAP_ANONYMOUS, -1, off_start) == MAP_FAILED) goto error;
