...
hidden void _dlstart_c(size_t *sp, size_t *dynv) {
  ...
  if (dynv) {
    ...
    while (phnum-- && ph->p_type != PT_DYNAMIC) ph = (void *)((size_t)ph + phent);
  ...
  for (; rel_size; rel += 3, rel_size -= 3 * sizeof(size_t)) {
    ...
    if (R_TYPE(rel[1]) == REL_FUNCDESC_VAL) {
      *rel_addr += segs[rel_addr[1]].addr - segs[rel_addr[1]].p_vaddr + syms[R_SYM(rel[1])].st_value;
