...
static int checker(void *p) {
  struct ctx *c = p;
  int ret;
  if (__syscall(SYS_setregid, __syscall(SYS_getegid), -1) || __syscall(SYS_setreuid, __syscall(SYS_geteuid), -1)) __syscall(SYS_exit, 1);
...
int faccessat(int fd, const char *filename, int amode, int flag) {
  ...
  if (!flag || (getuid() == geteuid() && getgid() == getegid())) return syscall(SYS_faccessat, fd, filename, amode);
  ...
  if (pid < 0 || __syscall(SYS_read, p[0], &ret, sizeof ret) != sizeof(ret)) ret = -EBUSY;
