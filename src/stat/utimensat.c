...
int utimensat(int fd, const char *path, const struct timespec times[2], int flags) {
  int r;
  if (times && times[0].tv_nsec == UTIME_NOW && times[1].tv_nsec == UTIME_NOW) times = 0;
  ...
  if (SYS_utimensat == SYS_utimensat_time64 || !IS32BIT(s0) || !IS32BIT(s1))
    r = __syscall(SYS_utimensat_time64, fd, path, times ? ((long long[]){s0, ns0, s1, ns1}) : 0, flags);
  if (SYS_utimensat == SYS_utimensat_time64 || r != -ENOSYS) return __syscall_ret(r);
  if (!IS32BIT(s0) || !IS32BIT(s1)) return __syscall_ret(-ENOTSUP);
  r = __syscall(SYS_utimensat, fd, path, times ? ((long[]){s0, ns0, s1, ns1}) : 0, flags);
#else
  ...
  if (times) {
    ...
    for (i = 0; i < 2; i++) {
      if (times[i].tv_nsec >= 1000000000ULL) {
        if (NS_SPECIAL(times[i].tv_nsec)) return __syscall_ret(-ENOSYS);
