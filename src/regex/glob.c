...
static int do_glob(char *buf, size_t pos, int type, char *pat, int flags, int (*errfunc)(const char *path, int err), struct match **tail) {
  ...
  if (!*pat) {
    ...
    if (!type && lstat(buf, &st)) {
      if (errno != ENOENT && (errfunc(buf, errno) || (flags & GLOB_ERR))) return GLOB_ABORTED;
      return 0;
    }
    if (append(tail, buf, pos, (flags & GLOB_MARK) && type == DT_DIR)) return GLOB_NOSPACE;
    return 0;
  }
  ...
  while (errno = 0, de = readdir(dir)) {
    ...
    if (p2 && de->d_type && de->d_type != DT_DIR && de->d_type != DT_LNK) continue;
...
static int expand_tilde(char **pat, char *buf, size_t *pos) {
  ...
  *pat = name_end;
  ...
  if (!home) {
    ...
    switch (*p ? getpwnam_r(p, &pw, buf, PATH_MAX, &res) : getpwuid_r(getuid(), &pw, buf, PATH_MAX, &res)) {
      case ENOMEM:
        return GLOB_NOSPACE;
      case 0:
        if (!res) default:
...
int glob(const char *restrict pat, int flags, int (*errfunc)(const char *path, int err), glob_t *restrict g) {
  ...
  if (*pat) {
    ...
    size_t pos = 0;
    char *s = p;
    if ((flags & (GLOB_TILDE | GLOB_TILDE_CHECK)) && *p == '~') error = expand_tilde(&s, buf, &pos);
    if (!error) error = do_glob(buf, pos, 0, s, flags, errfunc, &tail);
    free(p);
  }
