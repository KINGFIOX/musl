...
static void bcast_barrier(int s) { sem_post(&barrier_sem); }
...
int __membarrier(int cmd, int flags) {
  ...
  if (r && cmd == MEMBARRIER_CMD_PRIVATE_EXPEDITED && !flags) {
    ...
    struct sigaction sa = {.sa_flags = SA_RESTART | SA_ONSTACK, .sa_handler = bcast_barrier};
    memset(&sa.sa_mask, -1, sizeof sa.sa_mask);
    if (!__libc_sigaction(SIGSYNCCALL, &sa, 0)) {
      for (td = self->next; td != self; td = td->next) __syscall(SYS_tkill, td->tid, SIGSYNCCALL);
      for (td = self->next; td != self; td = td->next) sem_wait(&barrier_sem);
      r = 0;
