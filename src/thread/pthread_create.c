...
void __tl_lock(void) {
  ...
  while ((val = a_cas(&__thread_list_lock, 0, tid))) __wait(&__thread_list_lock, &tl_lock_waiters, val, 0);
}
...
_Noreturn void __pthread_exit(void *result) {
  ...
  while ((rp = self->robust_list.head) && rp != &self->robust_list.head) {
    ...
    if (cont < 0 || waiters) __wake(&m->_m_lock, 1, priv);
  }
  __vm_unlock();
...
static int start(void *p) {
  ...
  if (state) {
    if (a_cas(&args->control, 1, 2) == 1) __wait(&args->control, 0, 2, 1);
    if (args->control) {
...
int __pthread_create(pthread_t *restrict res, const pthread_attr_t *restrict attrp, void *(*entry)(void *), void *restrict arg) {
  ...
  unsigned flags = CLONE_VM | CLONE_FS | CLONE_FILES | CLONE_SIGHAND | CLONE_THREAD | CLONE_SYSVSEM | CLONE_SETTLS | CLONE_PARENT_SETTID |
  ...
  self = __pthread_self();
  if (!libc.threaded) {
    for (FILE *f = *__ofl_lock(); f; f = f->next) init_file_lock(f);
    __ofl_unlock();
  ...
  args->sig_mask[(SIGCANCEL - 1) / 8 / sizeof(long)] &= ~(1UL << ((SIGCANCEL - 1) % (8 * sizeof(long))));
  ...
  if (ret < 0) {
    ...
    if (a_swap(&args->control, ret ? 3 : 0) == 2) __wake(&args->control, 1, 1);
    if (ret) __wait(&args->control, 0, 3, 0);
  }
