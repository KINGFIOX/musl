...
#include "dynlink.h"
#undef CRTJMP
#define CRTJMP(pc, sp) __asm__ __volatile__("mov.l @%0+,r0 ; mov.l @%0,r12 ; jmp @r0 ; mov %1,r15" : : "r"(pc), "r"(sp) : "r0", "memory")
...
void __unmapself(void *base, size_t size) {
  if (__sh_nommu)
    __unmapself_sh_nommu(base, size);
  else
    __unmapself_sh_mmu(base, size);
}
