...
static void setsr(unsigned v) { __asm__ __volatile__("fmove.l %0,%%fpsr" : : "dm"(v)); }
...
static void setcr(unsigned v) { __asm__ __volatile__("fmove.l %0,%%fpcr" : : "dm"(v)); }
...
int fetestexcept(int mask) { return getsr() & mask; }

int fegetround(void) { return getcr() & FE_UPWARD; }
...
int fesetenv(const fenv_t *envp) {
  ...
  if (envp == FE_DFL_ENV) envp = &default_env;
  setcr(envp->__control_register);
  setsr(envp->__status_register);
  __asm__ __volatile__("fmove.l %0,%%fpiar" : : "dm"(envp->__instruction_address));
  return 0;
}
