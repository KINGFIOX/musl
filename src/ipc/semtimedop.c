...
#if !defined(SYS_semtimedop) && !defined(SYS_ipc) || SYS_semtimedop == SYS_semtimedop_time64
#define NO_TIME32 1
#else
#define NO_TIME32 0
#endif
...
int semtimedop(int id, struct sembuf *buf, size_t n, const struct timespec *ts) {
  ...
  int r = -ENOSYS;
  if (NO_TIME32 || !IS32BIT(s)) r = __syscall(SYS_semtimedop_time64, id, buf, n, ts ? ((long long[]){s, ns}) : 0);
