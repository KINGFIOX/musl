...
#define SIGPT_SET ((sigset_t *)(const unsigned long[_NSIG / 8 / sizeof(long)]){[sizeof(long) == 4] = 3UL << (32 * (sizeof(long) > 4))})
...
static inline void __wake(volatile void *addr, int cnt, int priv) {
  ...
  __syscall(SYS_futex, addr, FUTEX_WAKE | priv, cnt) != -ENOSYS || __syscall(SYS_futex, addr, FUTEX_WAKE, cnt);
}
