...
int timer_settime(timer_t t, int flags, const struct itimerspec *restrict val, struct itimerspec *restrict old) {
  ...
  int r = -ENOSYS;
  if (SYS_timer_settime == SYS_timer_settime64 || !IS32BIT(is) || !IS32BIT(vs) || (sizeof(time_t) > 4 && old))
    r = __syscall(SYS_timer_settime64, t, flags, ((long long[]){is, ins, vs, vns}), old);
  if (SYS_timer_settime == SYS_timer_settime64 || r != -ENOSYS) return __syscall_ret(r);
  if (!IS32BIT(is) || !IS32BIT(vs)) return __syscall_ret(-ENOTSUP);
  long old32[4];
  r = __syscall(SYS_timer_settime, t, flags, ((long[]){is, ins, vs, vns}), old32);
  if (!r && old) {
