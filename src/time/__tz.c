...
static uint32_t zi_read32(const unsigned char *z) { return (unsigned)z[0] << 24 | z[1] << 16 | z[2] << 8 | z[3]; }
...
static void do_tzset() {
  ...
  if (*s != ':') {
    ...
    if (p != s && (*p == '+' || *p == '-' || isdigit(*p) || !strcmp(dummy_name, "UTC") || !strcmp(dummy_name, "GMT"))) posix_form = 1;
  }
...
const char *__tm_to_tzname(const struct tm *tm) {
  ...
  LOCK(lock);
  do_tzset();
  if (p != __utc && p != __tzname[0] && p != __tzname[1] && (!zi || (uintptr_t)p - (uintptr_t)abbrevs >= abbrevs_end - abbrevs)) p = "";
  UNLOCK(lock);
  return p;
}
