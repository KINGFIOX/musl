...
static void processblock(struct sha256 *s, const uint8_t *buf) {
  ...
  for (; i < 64; i++) W[i] = R1(W[i - 2]) + W[i - 7] + R0(W[i - 15]) + W[i - 16];
  a = s->h[0];
  b = s->h[1];
  c = s->h[2];
  d = s->h[3];
...
static void sha256_update(struct sha256 *s, const void *m, unsigned long len) {
  ...
  for (; len >= 64; len -= 64, p += 64) processblock(s, p);
  memcpy(s->buf, p, len);
}
...
static void hashmd(struct sha256 *s, unsigned int n, const void *md) {
  ...
  for (i = n; i > 32; i -= 32) sha256_update(s, md, 32);
...
static char *sha256crypt(const char *key, const char *setting, char *output) {
  ...
  if (klen > KEY_MAX) return 0;

  /* setting: $5$rounds=n$salt$ (rounds=n$ and closing $ are optional) */
  if (strncmp(setting, "$5$", 3) != 0) return 0;
  salt = setting + 3;
  ...
  for (i = 0; i < SALT_MAX && salt[i] && salt[i] != '$'; i++) /* reject characters that interfere with /etc/shadow parsing */
    if (salt[i] == '\n' || salt[i] == ':') return 0;
  slen = i;
  ...
  sha256_init(&ctx);
  for (i = 0; i < klen; i++) sha256_update(&ctx, key, klen);
  sha256_sum(&ctx, kmd);

  /* DS = sha(repeat-salt) */
  sha256_init(&ctx);
  for (i = 0; i < 16 + md[0]; i++) sha256_update(&ctx, salt, slen);
  sha256_sum(&ctx, smd);
  ...
  for (i = 0; i < r; i++) {
    ...
    if (i % 2)
      ...
      sha256_update(&ctx, md, sizeof md);
    if (i % 3) sha256_update(&ctx, smd, slen);
    if (i % 7) hashmd(&ctx, klen, kmd);
    if (i % 2)
      sha256_update(&ctx, md, sizeof md);
    else
      hashmd(&ctx, klen, kmd);
  ...
  static const unsigned char perm[][3] = {0, 10, 20, 21, 1, 11, 12, 22, 2, 3, 13, 23, 24, 4, 14, 15, 25, 5, 6, 16, 26, 27, 7, 17, 18, 28, 8, 9, 19, 29};
  for (i = 0; i < 10; i++) p = to64(p, (md[perm[i][0]] << 16) | (md[perm[i][1]] << 8) | md[perm[i][2]], 4);
...
char *__crypt_sha256(const char *key, const char *setting, char *output) {
  ...
  if (!p || q != testbuf || memcmp(testbuf, testhash, sizeof testhash)) return "*";
  return p;
}
