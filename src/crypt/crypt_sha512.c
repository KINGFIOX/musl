...
static void sha512_update(struct sha512 *s, const void *m, unsigned long len) {
  ...
  for (; len >= 128; len -= 128, p += 128) processblock(s, p);
  memcpy(s->buf, p, len);
}
...
static void hashmd(struct sha512 *s, unsigned int n, const void *md) {
  ...
  for (i = n; i > 64; i -= 64) sha512_update(s, md, 64);
...
static char *sha512crypt(const char *key, const char *setting, char *output) {
  ...
  if (strncmp(setting, "$6$", 3) != 0) return 0;
  salt = setting + 3;
  ...
  sha512_init(&ctx);
  for (i = 0; i < klen; i++) sha512_update(&ctx, key, klen);
  sha512_sum(&ctx, kmd);

  /* DS = sha(repeat-salt) */
  sha512_init(&ctx);
  for (i = 0; i < 16 + md[0]; i++) sha512_update(&ctx, salt, slen);
  sha512_sum(&ctx, smd);
  ...
  for (i = 0; i < r; i++) {
    ...
    if (i % 2)
      ...
      sha512_update(&ctx, md, sizeof md);
    if (i % 3) sha512_update(&ctx, smd, slen);
...
#if 1
  static const unsigned char perm[][3] = {0,  21, 42, 22, 43, 1,  44, 2,  23, 3,  24, 45, 25, 46, 4,  47, 5,  26, 6,  27, 48,
  ...
  for (i = 0; i < 21; i++) p = to64(p, (md[perm[i][0]] << 16) | (md[perm[i][1]] << 8) | md[perm[i][2]], 4);
#else
