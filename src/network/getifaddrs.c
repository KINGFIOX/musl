...
static void copy_addr(struct sockaddr **r, int af, union sockany *sa, void *addr, size_t addrlen, int ifindex) {
  ...
  switch (af) {
    ...
    case AF_INET6:
      ...
      len = 16;
      if (IN6_IS_ADDR_LINKLOCAL(addr) || IN6_IS_ADDR_MC_LINKLOCAL(addr)) sa->v6.sin6_scope_id = ifindex;
      break;
    default:
      return;
  }
...
static int netlink_msg_to_ifaddr(void *pctx, struct nlmsghdr *h) {
  ...
  if (h->nlmsg_type == RTM_NEWLINK) {
    ...
    if (ifs->ifa.ifa_addr) gen_netmask(&ifs->ifa.ifa_netmask, ifa->ifa_family, &ifs->netmask, ifa->ifa_prefixlen);
  }
