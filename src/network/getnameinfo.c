...
static void mkptr4(char *s, const unsigned char *ip) { sprintf(s, "%d.%d.%d.%d.in-addr.arpa", ip[3], ip[2], ip[1], ip[0]); }
...
static void reverse_hosts(char *buf, const unsigned char *a, unsigned scopeid, int family) {
  ...
  while (fgets(line, sizeof line, f)) {
    ...
    if (!*p) continue;
    *p++ = 0;
    if (__lookup_ipliteral(&iplit, line, AF_UNSPEC) <= 0) continue;
    ...
    if (memcmp(a, iplit.addr, 16) || iplit.scopeid != scopeid) continue;
...
static int dns_parse_callback(void *c, int rr, const void *data, int len, const void *packet, int plen) {
  ...
  if (__dn_expand(packet, (const unsigned char *)packet + plen, data, c, 256) <= 0) *(char *)c = 0;
  return 0;
}
...
int getnameinfo(const struct sockaddr *restrict sa, socklen_t sl, char *restrict node, socklen_t nodelen, char *restrict serv, socklen_t servlen, int flags) {
  ...
  if (node && nodelen) {
    ...
    if (!*buf) {
      ...
      if (scopeid) {
        ...
        if (!(flags & NI_NUMERICSCOPE) && (IN6_IS_ADDR_LINKLOCAL(a) || IN6_IS_ADDR_MC_LINKLOCAL(a))) p = if_indextoname(scopeid, tmp + 1);
        if (!p) p = itoa(num, scopeid);
        *--p = '%';
        strcat(buf, p);
      }
    }
  ...
  if (serv && servlen) {
    ...
    buf[0] = 0;
    if (!(flags & NI_NUMERICSERV)) reverse_services(buf, port, flags & NI_DGRAM);
    if (!*p) p = itoa(num, port);
    if (strlen(p) >= servlen) return EAI_OVERFLOW;
    strcpy(serv, p);
  }
