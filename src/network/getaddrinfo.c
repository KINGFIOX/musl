...
int getaddrinfo(const char *restrict host, const char *restrict serv, const struct addrinfo *restrict hint, struct addrinfo **restrict res) {
  ...
  if (hint) {
    ...
    const int mask = AI_PASSIVE | AI_CANONNAME | AI_NUMERICHOST | AI_V4MAPPED | AI_ALL | AI_ADDRCONFIG | AI_NUMERICSERV;
  ...
  if (flags & AI_ADDRCONFIG) {
    ...
    static const struct sockaddr_in lo4 = {.sin_family = AF_INET, .sin_port = 65535, .sin_addr.s_addr = __BYTE_ORDER == __BIG_ENDIAN ? 0x7f000001 : 0x0100007f};
    ...
    for (i = 0; i < 2; i++) {
      ...
      int s = socket(tf[i], SOCK_CLOEXEC | SOCK_DGRAM, IPPROTO_UDP);
      if (s >= 0) {
        int cs;
  ...
  for (k = i = 0; i < naddrs; i++)
    for (j = 0; j < nservs; j++, k++) {
      ...
      out[k].ai = (struct addrinfo){.ai_family = addrs[i].family,
                                    ...
                                    .ai_addrlen = addrs[i].family == AF_INET ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6),
