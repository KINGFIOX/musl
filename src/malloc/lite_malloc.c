...
static void *__simple_malloc(size_t n) {
  ...
  if (!n) n++;
  while (align < n && align < ALIGN) align += align;
  ...
  if (n > end - cur) {
    ...
    if (brk == end && req < SIZE_MAX - brk && !traverses_stack_p(brk, brk + req) && __syscall(SYS_brk, brk + req) == brk + req) {
      brk = end += req;
    } else {
      int new_area = 0;
...
static void *default_malloc(size_t n) { return __libc_malloc_impl(n); }
