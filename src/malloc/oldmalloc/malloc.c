...
static const unsigned char bin_tab[60] = {
    32, 33, 34, 35, 36, 36, 37, 37, 38, 38, 39, 39, 40, 40, 40, 40, 41, 41, 41, 41, 42, 42, 42, 42, 43, 43, 43, 43, 44, 44,
    44, 44, 44, 44, 44, 44, 45, 45, 45, 45, 45, 45, 45, 45, 46, 46, 46, 46, 46, 46, 46, 46, 47, 47, 47, 47, 47, 47, 47, 47,
};
...
static void *__expand_heap(size_t *pn) {
  ...
  if (n < SIZE_MAX - brk && !traverses_stack_p(brk, brk + n) && __syscall(SYS_brk, brk + n) == brk + n) {
    *pn = n;
    brk += n;
    return (void *)(brk - n);
  }
...
static void bin_chunk(struct chunk *self, int i) {
  ...
  self->next->prev = self;
  self->prev->next = self;
  if (self->prev == BIN_TO_CHUNK(i)) a_or_64(&mal.binmap, 1ULL << i);
}
...
int __malloc_allzerop(void *p) { return IS_MMAPPED(MEM_TO_CHUNK(p)); }
...
void *realloc(void *p, size_t n) {
  ...
  if (IS_MMAPPED(self)) {
    ...
    if (base == (void *)-1) goto copy_realloc;
...
#else
    __mmap((void *)a, b - a, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
#endif
    errno = e;
  }
...
void __malloc_donate(char *start, char *end) {
  ...
  if (end - start <= OVERHEAD + align_start_up + align_end_down) return;
...
void __malloc_atfork(int who) {
  if (who < 0) {
    lock(mal.split_merge_lock);
    for (int i = 0; i < 64; i++) lock(mal.bins[i].lock);
  } else if (!who) {
    for (int i = 0; i < 64; i++) unlock(mal.bins[i].lock);
    unlock(mal.split_merge_lock);
  } else {
    for (int i = 0; i < 64; i++) mal.bins[i].lock[0] = mal.bins[i].lock[1] = 0;
