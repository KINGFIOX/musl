...
static locale_t do_newlocale(int mask, const char *name, locale_t loc) {
  ...
  for (int i = 0; i < LC_ALL; i++) {
    tmp.cat[i] = (!(mask & (1 << i)) && loc) ? loc->cat[i] : __get_locale(i, (mask & (1 << i)) ? name : "");
    if (tmp.cat[i] == LOC_MAP_FAILED) return 0;
  }
  ...
  if (!default_locale_init_done) {
    for (int i = 0; i < LC_ALL; i++) default_locale.cat[i] = __get_locale(i, "");
  ...
  if (!memcmp(&tmp, &default_ctype_locale, sizeof tmp)) return &default_ctype_locale;
